...
/**
 * universens-lead controller
 */

const { createCoreController } = require('@strapi/strapi').factories;
const mailer = require('../../../lib/mailer');

module.exports = createCoreController('api::universens-lead.universens-lead', ({ strapi }) => ({
  async create(ctx) {
    ...
    <div style="background-color: #f4f6f8; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; padding: 20px 0;">
      <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.05); overflow: hidden;">
        <div style="background-color: #9C27B0; height: 8px;"></div> <!-- Couleur Violet pour Universens -->
        <div style="padding: 25px;">
          <p style="color: #546E7A; font-size: 16px; margin-top: 0;">Bonjour Nicolas,</p>
          <p style="color: #37474F; line-height: 1.6;">Voici un nouveau lead Universens :</p>
          <div style="margin-top: 25px; border: 1px solid #ECEFF1; border-radius: 6px;">
            ...
            <div style="padding: 12px 15px; border-bottom: 1px solid #ECEFF1; display: flex; align-items: center;">
              ...
              <div>
                <div style="font-size: 10px; text-transform: uppercase; color: #90A4AE; font-weight: bold; letter-spacing: 0.5px;">Source</div>
                <div style="font-size: 14px; color: #9C27B0; font-weight: 600; margin-top: 1px;">
                  ${data.sourceForm || 'Universens'}
                </div>
              </div>
            </div>
            ...
            <div style="padding: 12px 15px; border-bottom: 1px solid #ECEFF1; display: flex; align-items: center;">
              ...
              <div>
                ...
                <div style="font-size: 14px; color: #263238; font-weight: 600; margin-top: 1px;">${data.raisonSociale || 'Non renseigné'}</div>
              </div>
            </div>
            ...
            <div style="padding: 12px 15px; border-bottom: 1px solid #ECEFF1; display: flex; align-items: center;">
              ...
              <div>
                ...
                <div style="font-size: 14px; color: #9C27B0; font-weight: 600; margin-top: 1px;">
                  <a href="mailto:${data.email}" style="color: #9C27B0; text-decoration: none;">${data.email}</a>
                </div>
              </div>
            </div>
            ...
            <div style="padding: 12px 15px; display: flex; align-items: center;">
              <div style="width: 25px; font-size: 16px; margin-right: 15px;">✅</div>
              <div>
                <div style="font-size: 10px; text-transform: uppercase; color: #90A4AE; font-weight: bold; letter-spacing: 0.5px;">Consentement</div>
                <div style="font-size: 14px; color: #263238; font-top: 1px;">Transmission données : ${data.accepteTransmissionDonnees || 'Non renseigné'}</div>
              </div>
            </div>
          </div>
          ...
          <div style="margin-top: 25px;">
            ...
            <div style="background-color: #F3E5F5; border-left: 4px solid #9C27B0; padding: 15px; border-radius: 4px; color: #4A148C; font-style: italic; line-height: 1.5; font-size: 14px;">${(data.message || 'Aucun message').replace(/\n/g, '<br>')}</div>
          </div>
    ...
    try {
      await mailer.createDraft({ subject: `Lead Universens - ${data.nom}`, html: htmlContent });
